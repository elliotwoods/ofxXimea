...
#define MM40_HWN2_SHIFT_CHIP   20   // position of chip id in HWN2 number
#define MM40_HWN2_SHIFT_MOSAIC 16   // position of mosaic id in HWN2 number
#define MM40_HWN2_SHIFT_RELEASE 4   // position of release id in HWN2 number
#define MM40_HWN2_SHIFT_ADC     0   // position of adc id in HWN2 number
...
#define MM40_HWN2_CMV2000_REV2  0x220       // CMOSIS 2.0Mpix rev2	sensor without PLL inside 
#define MM40_HWN2_CMV4000_REV2  0x240       // CMOSIS 4.0Mpix rev2	sensor without PLL inside 
...
#define	MM40_HWN2_HW_REL		(((dw)>>MM40_HWN2_SHIFT_RELEASE) & 0xF)
...
typedef enum {
	...
	FAMILY_MQ				= 0x005			//!< USB 3.0 cameras
	// etc
} FAMILY;
...
typedef enum {
	...
	LED_STREAMING			= 0x003,		//!< set led to blink if data streaming
	LED_INTEGRATION			= 0x004,		//!< set led to blink if sensor integration time
	LED_ISACQ				= 0x005,		//!< set led to blink if device busy/not busy
	LED_NIC					= 0x006,		//!< set led to blink if link is ok
...
typedef	struct {
	...
	DWORD	curX0;			//!< Partial readout Y origin used - covers some guard areas for interpolation
	DWORD	curCX;			//!< Partial readout Y extention
	DWORD	curY0;			//!< Partial readout X origin used - covers full width
	DWORD	curCY;			//!< Partial readout X extention
	DWORD	curBinX;		//!< Current data X binning
	DWORD	curBinY;		//!< Current data Y binning
	DWORD	curBPP;			//!< Current data bits per pixel
	DWORD	curMTX;			//!< Color matrix code
...
/**
   ...
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmRetrieveRaw(IN HANDLE hDevice, LPVOID * output, DWORD * data_size);
...
/**
   ...
   @param[in] width				width of input source image	
   @param[in] height			height of input source image	
   @param[in] bpp				bits per pixel 
   @param[out] output			pointer to raw data
   @param[in] data_size			data size
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmSetSrcData(LPWORD input, DWORD width, DWORD height, DWORD bpp, LPVOID * output, DWORD * data_size);
...
/**
   ...
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmSetRawData(LPVOID input);

/**
   \brief Applies filter to raw data

	Applies filter, specified by pointer to raw data.

   @param[in] lpFlt			pointer to LPMMFILTER structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmSetupFilterRaw (IN LPMMFILTER lpFlt );

/**
   \brief Applies up CE (contrast enhancement) to raw data_size
   
   This function is similar to \ref mmSetupCE, but with only difference that it defines the width of dynamic range and therefore contrast enhancement that will be applied to raw data, rather than to the processed one.

   @param[in] dwLow			dynamic range lower bound
   @param[in] dwHigh		dynamic range upper bound
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmSetupCERaw (IN DWORD dwLow, IN DWORD dwHigh );

/**
   \brief Retrieves specified raw frame.

	This function retrieves the frame raw data into MMGETFRAMEEX structure.

	\code
	//device (HANDLE hDevice) should be already initialized here
	MMGETFRAMEEX lpGFex;
	int result = mmRetrieveRawFrameEx(hDevice, &lpGFex);
	\endcode

   @param[in,out] lpGF			address of a MMGETFRAMEEX structure where the frame data will be stored on return 
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmRetrieveRawFrameEx	(INOUT LPMMGETFRAMEEX lpGF );

/**
   \brief Processes raw frame
   
   \warning Not implemented

   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmProcessRawFrame ();

/**
   \brief Retrieves bitmap acquired from raw data_size
   
   Allows the user to access bitmap acquired from raw data. It can be accessed through two pointers, pointer to \ref MMGETBITMAP and pointer to \ref MMGETBITMAPOPT structures 

   @param[in,out] lpGB			LPMMGETBITMAP, pointer to MMGETBITMAP structure
   @param[in,out] lpGBopt			LPMMGETBITMAPOPT, pointer to MMGETBITMAPOPT structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmRetrieveBitmapAdvRaw ( INOUT LPMMGETBITMAP lpGB,	INOUT LPMMGETBITMAPOPT lpGBopt );
...
/**
   ...
   @param[in,out] lpGB			LPMMGETBITMAP, pointer to MMGETBITMAP structure
   @param[in,out] lpGBopt			LPMMGETBITMAPOPT, pointer to MMGETBITMAPOPT structure
   @param[in]	  noise				LPNOISE_PARAMS, pointer to \ref NOISE_PARAMS structure
   @return MM40_OK on success, error value otherwise.
   @return MM40_INVALID_DATA
   @return MM40_INVALID_ARG
   @return MM40_MEMORY_ALLOCATION
 */
MM40_API MM40_RETURN __cdecl mmRetrieveBitmapDnRaw ( INOUT LPMMGETBITMAP lpGB,	INOUT LPMMGETBITMAPOPT lpGBopt, IN LPNOISE_PARAMS noise );


/**
   \brief Returns raw filter

	Allows the user to access (retrieve or apply ???) filter that was applied to raw data.

   @param[in] lpFlt			LPMMFILTER, pointer to MMFILTER structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmGetFilterRaw (IN LPMMFILTER lpFlt );
...
/**
   \brief Frees raw bitmap
   
   Release bitmap acquired from raw data and frees the alocated space.

   @param[in,out] lpGB			LPMMGETBITMAP, pointer to MMGETBITMAP structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmFreeBitmapRaw( INOUT LPMMGETBITMAP lpGB );

/**
   \brief Returns raw means
   
   Calculates and return R (red), GR (green in red), GB (green in blue) and B (blue) mean values within specific user-defined region of the bitmap acquired from raw data.
   ...
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmGetRawMeans ( IN DWORD dwX0,	IN DWORD dwY0, IN DWORD dwCX, IN DWORD dwCY, INOUT float * fMeans );

/**
   \brief Applies shading correction to raw data
   
   Applies shading correction to raw data

   @param[in] lpMMS				shading structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmSetShadingRaw (IN LPMMSHADING lpMMS);
...
/**
   ...
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmCalculateShadingRaw (INOUT LPMMSHADING lpMMS, DWORD dwCX, DWORD dwCY, LPWORD pBlack, LPWORD pWhite );

/**
   \brief Releases shading allocated resources
   
   AReleases shading allocated resources

   @param[in] lpMMS				shading structure
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmReleaseShadingRaw (IN LPMMSHADING lpMMS);
//*****************************************************************
